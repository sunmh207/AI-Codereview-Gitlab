...
def send_notification(content, msg_type='text', title="通知", is_at_all=False, project_name=None, url_slug=None,
                      webhook_data: dict={}, mentioned_list=None, msg_category=None, project_config=None):
    """
    ...
    """
    # 钉钉推送
    dingtalk_notifier = DingTalkNotifier(project_config=project_config)
    ...
    # 企业微信推送
    wecom_notifier = WeComNotifier(project_config=project_config)
    ...
    # 飞书推送
    feishu_notifier = FeishuNotifier(project_config=project_config)
    feishu_notifier.send_message(content=content, msg_type=msg_type, title=title, is_at_all=is_at_all,
                                 project_name=project_name, url_slug=url_slug, msg_category=msg_category)

    # 额外自定义webhook通知
    extra_webhook_notifier = ExtraWebhookNotifier(project_config=project_config)
    system_data = {
        "content": content,
        "msg_type": msg_type,
        "title": title,
